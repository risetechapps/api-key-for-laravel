    ...
    <div class="space-y-6">
                ...
                <div class="flex flex-wrap gap-3">
                    <select
                        ...
                        class="px-4 py-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white
                         dark:bg-slate-800 text-slate-900 dark:text-white focus:border-indigo-500
                         transition-all">
                    ...
                    <select
                        ...
                        class="px-4 py-3 rounded-xl border-2 border-slate-200 dark:border-slate-700
                         bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-indigo-500
                          transition-all">
        ...
        <Card>
                ...
                <table class="w-full">
                    ...
                    <tr class="border-b border-slate-200 dark:border-slate-700">
                        <th class="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase
                            tracking-wider py-4 px-4">Data/Hora
                        </th>
                        <th class="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase
                             tracking-wider py-4 px-4">Endpoint
                        </th>
                        <th class="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase
                             tracking-wider py-4 px-4">Método
                        </th>
                        <th class="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase
                             tracking-wider py-4 px-4">Status
                        </th>
                        <th class="text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase
                             tracking-wider py-4 px-4">Ações
                        </th>
                    </tr>
                    </thead>
                    ...
                    <tr v-else-if="filteredRequests.length === 0">
                        <td colspan="5" class="py-12 text-center">
                            <div class="flex flex-col items-center">
                                <div class="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center
                                     justify-center mb-4">
                    ...
                    >
                        ...
                        <td class="py-4 px-4 text-right">
                            <button
                                ...
                                dark:hover:text-indigo-300 text-sm font-medium">
                                Detalhes
                            </button>
                        </td>
                    </tr>
            ...
            <div class="sm:hidden space-y-3">
                <div v-if="loading" v-for="i in 5" :key="`m-loading-${i}`"
                     class="h-20 bg-slate-200 dark:bg-slate-700 rounded-xl animate-pulse"/>

                <p v-else-if="filteredRequests.length === 0"
                ...
                <div v-else v-for="request in paginatedRequests" :key="`m-${request.id}`"
                     class="p-4 rounded-xl border border-slate-200 dark:border-slate-700
                            bg-white dark:bg-slate-800 space-y-2">
                    ...
                    <p class="font-mono text-sm text-slate-900 dark:text-white break-all">
                        {{ request.endpoint }}
                    </p>
                    <div class="flex items-center justify-between">
                        ...
                        <button @click="showDetails(request)"
            ...
            <div v-if="filteredRequests.length > 0"
                ...
                <div class="flex items-center gap-2">
                    <button
                        ...
                        class="p-2 rounded-lg border border-slate-200 dark:border-slate-700 disabled:opacity-50
                         disabled:cursor-not-allowed hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                        <PhCaretLeft :size="18" class="text-slate-600 dark:text-slate-400"/>
                    </button>

                    <span class="text-sm text-slate-600 dark:text-slate-400 px-3">
        ...
        <Modal v-model="detailsModal" title="Detalhes da Requisição">
            <div v-if="selectedRequest" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <span
                            class="inline-flex mt-1 px-2.5 py-1 rounded-lg text-xs font-semibold"
                            :class="getMethodClass(selectedRequest.method)"
                        >
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <span
                                class="w-2.5 h-2.5 rounded-full"
                                :class="getStatusColor(selectedRequest.response_code)"
                            />
                            <span class="font-medium text-slate-900 dark:text-white">
                                {{ selectedRequest.response_code }}
                            </span>
                        </div>
                    </div>
                    <div class="col-span-2">
                        <p class="text-sm text-slate-500 dark:text-slate-400">Endpoint</p>
                        <p class="font-mono text-sm text-slate-900 dark:text-white mt-1 break-all">
                    ...
                    <div class="col-span-2">
                        ...
                        <p class="text-sm text-slate-900 dark:text-white mt-1">
                            {{ formatDateLong(selectedRequest.requested_at) }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <template #footer>
                <Button variant="outline" @click="detailsModal = false">Fechar</Button>
            </template>
        </Modal>
    </div>
</template>
...
const formatDate = (date) => {
    return format(new Date(date.replace(' ', 'T')), 'dd/MM/yyyy HH:mm', {locale: ptBR});
}

const formatDateLong = (date) => {
    return format(new Date(date.replace(' ', 'T')), "dd 'de' MMMM 'de' yyyy 'às' HH:mm:ss", {locale: ptBR});
}
